...
def bank_addUser(account,username,password,country,province,street,door,money):
    # 判断是否已满
    sql = "select count(*) from yonghu"
    param = []
    data = select(sql,param,mode="one")# (4,)
    if data[0] > 100:
        return 3

    sql1 = "select * from yonghu where username  = %s"
    param1 = [username]
    ...
    # 正常开户
    sql2 = "insert into yonghu values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
...
def cun_money(account,addmoney):
    sql4 = 'select * from yonghu where account = %s'
    param4 = [account]
    abc = select(sql4, param4,mode='all')
    if len(abc) > 0:
        sql3 = 'update yonghu set money = money  + %s'
        param3 = [addmoney]
        update(sql3,param3)
        return 1
    else:
...
def add_money(account,money,password):
    sql5 = 'select * from yonghu where account  = %s'
    param5 = [account]
    a = select(sql5, param5,mode='all')
    sql1='select password from yonghu where account = %s and password = %s'
    param1 = [account,password]
    b=select(sql1, param1)
    sql2="select money from yonghu where account=%s and money >=%s"
    ...
    if len(a)==0:
        ...
        if len(b) == 0 :
            ...
            if len(c)==0:
                ...
                sql3 = 'update yonghu set money = money  - %s where account = %s'
...
def bank_cha(account,password):
    sql1='select * from yonghu where account = %s and password = %s'
    param1 = [account,password]
    b=select(sql1, param1)
    if  len(b) > 0 :
        return 1
...
def bank_chaxin():
    ...
    if addcha == 1:
        print("嘻嘻，查询成功")
        sql='select * from yonghu where account=%s'
        param=[account]
        a=select(sql, param)
        info = '''
            ------------个人信息查询结果-------------
...
def transfer_process(account1,password,account2,money):
    ...
    sql6 = 'select * from yonghu where account  = %s'
    param6 = [account2]
    ...
    if len(a)==0 or len(b)==0:
        ...
        if len(d)==0:
            ...
            sql='select money from yonghu where account = %s and money > %s'
            param=[account1,money]
            c=select(sql, param,mode='all')
            if len(c)==0:
                return 3
            else:
                sql2='update yonghu set money = money  + %s where account=%s'
